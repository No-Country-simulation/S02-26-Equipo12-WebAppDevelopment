...
export async function createMeasurements(
  ...
  for (const [fieldName, measurementTypeId] of Object.entries(MEASUREMENT_TYPE_IDS)) {
    ...
    payload.push({ measurementTypeId, value });
  }
  ...
  return fetch(`${API_BASE}/riders/${riderId}/measurements`, {
    method: "POST",
...
//   body: Partial<MeasurementsPayload>,
// ): Promise<Response> {
...
// export function mountRiderMeasurementsForm(
//   formId = "rider-measurements-form",
// ): void {
...
//       existingMeasurements.map((m) => [m.measurementTypeId, m.id]),
//     );
